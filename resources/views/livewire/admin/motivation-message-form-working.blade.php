<div>
    ...
    @if($editingRecordId)
        <div class="mb-4 text-sm text-gray-500">
            Debug: EditingID: {{ $editingRecordId }}<br>
            Messages length: {{ strlen($messages) }}<br>
            Categories count: {{ count($categories) }}<br>
            Records count: {{ count($existingRecords) }}
        </div>
    @endif
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form wire:submit.prevent="save" wire:key="form-{{ $editingRecordId ?? 'new' }}">
            {{-- Category select --}}
            <div class="row mb-3">
                <div class="col">
                ...
                @if(!empty($availableExpressionStyles))
                    <div class="col">
                        <label class="form-label">Expression Style</label>
                            ...
                            <tbody>
                                @forelse ($existingRecords as $record)
                                    <tr>
                                        ...
                                        <td>
                                            <button wire:click="editCategory('{{ $record['_id'] }}')" class="btn btn-sm btn-primary">
                                                <i class="fas fa-edit"></i> Edit
                                            </button>
                                            <button wire:click="generateAudioForRecord('{{ $record['_id'] }}')" class="btn btn-sm btn-secondary ms-2">
